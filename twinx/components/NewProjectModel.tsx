...
const NewProjectModal: FC<NewProjectModalProps> = ({ isOpen, onClose, userId }) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setVideoFile(file);
    setVideoUrl(URL.createObjectURL(file)); // preview only
  };

  // Upload to Azure Blob (used inside handleSubmit)
  const uploadVideoToAzure = async (file: File) => {
    if (!file) return null;

    setIsGenerating(true);

    try {
      // 1️⃣ Ask server for SAS URL
      ...
      if (!data?.success || !data?.uploadUrl || !data?.blobUrl) {
        console.error("Failed to get SAS URL:", data);
      ...
      // 2️⃣ PUT the file directly to Azure
      ...
      showNotification("Video upload complete", "normal");
      console.log("✅ Uploaded video to:", blobUrl);
      return blobUrl;
    } catch (err) {
      console.error("❌ Upload error:", err);
      showNotification("Upload error: " + (err as Error).message, "error");
      return null;
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    // ✅ validate first (fixes your TS error)
    if (!title || !videoFile || !userId) {
      showNotification("Please provide a title and a video file.");
      return;
    }
  
    setIsGenerating(true);
    // ⬆️ Upload video now that we’re sure videoFile is NOT null
    const uploadedVideoUrl = await uploadVideoToAzure(videoFile);
    if (!uploadedVideoUrl) {
          ...
          <div className="bg-[#262629] rounded-lg shadow-xl w-full max-w-lg border border-[#3A3A3C] transform transition-all scale-95 animate-scale-in">
              ...
              <form onSubmit={handleSubmit}>
                  ...
                  <div className="p-4 bg-[#1C1C1E] border-t border-[#3A3A3C] flex justify-end gap-3">
                      ...
                      <button type="submit" disabled={isGenerating} className="bg-[#6366F1] text-white font-semibold py-2 px-4 rounded-md hover:bg-opacity-90 transition-colors disabled:bg-opacity-50 disabled:cursor-not-allowed">
