// /twinx/modals/NewProjectModal.tsx
'use client';

import { useState, useEffect, useRef } from 'react';
import { X, UploadCloud } from 'lucide-react';
import { generateTwinxId } from '../lib/helpers';
...
interface NewProjectModalProps {
    ...
    onProjectCreated: (projectId: string, videoUrl: string) => void;
}

const NewProjectModal = ({ isOpen, onClose, userId, showNotification, onProjectCreated }: NewProjectModalProps) => {
    const [title, setTitle] = useState('');
    const [videoFile, setVideoFile] = useState<File | null>(null);
    const [thumbnail, setThumbnail] = useState('');
    const [videoUrl, setVideoUrl] = useState('');
    const [twinxid, setTwinxid] = useState('');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const docRef = await addDoc(collection(db, projectsCollectionPath), newProject);
            showNotification("Digital Twin created successfully!");
            onProjectCreated(docRef.id, videoUrl); // Pass session URL
        } catch (error) {
